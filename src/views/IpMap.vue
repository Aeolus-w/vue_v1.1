      ...
      <div class="roll-list">
        <div class="inner-list" id="list">
          <table width="100%" style="table-layout:fixed;" class="table table-hover table-responsive table-condensed table-striped">
            <tbody>
              <tr v-for="(value, key) in ipValue" :key="key">
                <td>{{ value.split(':')[1] }}</td>
                <td>{{ key }}</td>
                <td>{{ geoData[key] }}</td>
...
export default {
  ...
  methods: {
    initMap() {
      const { ipValue, geoData, myGeo } = this;
      $('#ipmap').width($(window).width());
      $('#ipmap').height($(window).height());

      const myChart = echarts.init(document.getElementById('ipmap'));
      const option = {
      ...
      for (let i = 0; i < myGeo.length; i++) {
        option.series.push({
          name: 'data',
          type: 'scatter',
          coordinateSystem: 'geo',
          data: [
            { name: myGeo[i], value: [geoData[myGeo[i]][0], geoData[myGeo[i]][1], ipValue[myGeo[i]] ? parseInt(ipValue[myGeo[i]].split(':')[0]) : 0] }
          ],
    ...
    initScroll() {
      let listHeight = $('.inner-list').height();
      let containerHeight = $('.roll-list').height();
      let index = 0;
      let maxIndex = Math.ceil(listHeight / containerHeight) - 1;
      ...
      setInterval(() => {
        ...
        $('.inner-list').animate({ top: -index * containerHeight }, 1000);
      }, 3000);
    }
  }
};
